  ...
  <v-container fluid grid-list-xl>
    <v-layout row wrap>
      ...
      <v-flex xs3 v-for="item in users" :key="item.user.id">
        <employee-card :id="item.user.id" :firstName="item.user.first_name" :lastName="item.user.last_name" :gender="item.gender" :position="(item.position != null) ? item.position.name : 'No Position found'" :level="(item.position != null) ? item.position.level : 'No found level'" ></employee-card>
      </v-flex>
    </v-layout>
  </v-container>
</template>
